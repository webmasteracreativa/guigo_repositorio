...
let AnalyticsCover = createReactClass({
	...
	toggleDisableAnalytics: function(e) {
		...
		jQuery.getJSON(`${FormCraftGlobal.ajaxurl}action=formcraft_toggle_analytics&disable=${e.target.checked}`, function() {
			// this.refreshChart()
		}.bind(this))
	},
					...
					<div className='block-header'>
						<span className='block-title'>Form Analytics</span>
							...
							<label className='float-right'>
								...
								<span style={{ fontWeight: 500, margin: '5px 0 0px 6px', display: 'inline-block' }}>Disable Analytics</span>
							</label>
					</div>
					<div className='tr thead'>
						<div style={{ width: '40%' }}>
