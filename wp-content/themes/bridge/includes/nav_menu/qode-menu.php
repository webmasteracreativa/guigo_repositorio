		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide"){
					...
					if($wide_background == 'true'){
						$menu_type .= " wide_background";
						$menu_type .= $wide_menu == 'true' ? ' full_width_wide_menu' : '' ;
					}
				}elseif($item->type_menu == "wide_icons"){
					$menu_type .= " wide icons";
					if($wide_background == 'true'){
						$menu_type .= " icons wide_background";
						$menu_type .= $wide_menu == 'true' ? ' full_width_wide_menu' : '' ;
					}
				}else{
					$menu_type .= " narrow";
			...
			if($item->hide == ""){
				...
				if($item->featured_icon !== ""){
					$item_output .= '<span class="qode-featured-icon '. $item->featured_icon .'" aria-hidden="true"></span>';
				}
		...
		// add main/sub classes to li's and links
		 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				if($item->featured_icon !== ""){
					$featured_icon .= '<span class="qode-featured-icon '. $item->featured_icon .'" aria-hidden="true"></span>';
				}
