...
if($type == 'advanced'){
	...
	if($active == 'yes') {
		$html .= "<div class='active_text'><span>".$active_text."</span></div>";
	}
	...
	if(empty($title_tag_standard)){
		...
		$html .= "<li class='cell table_title'><" . $title_tag_standard . " class='qode_title_content_new'>".$title."</" . $title_tag_standard . ">";
	}
	...
	if($show_button == 'yes') {
		$html .="<li class='price_button'>";
		$html .= "<a itemprop='url' class='qbutton white $button_size' href='$link' target='$target'>".$button_text."</a>";
		$html .= "</li>"; //close li.price_button
	}
